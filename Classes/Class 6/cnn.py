...
'''
Load training and testing sets (numpy arrays)
'''
def load_data():
    (x_train, y_train), (x_test, y_test) = tf.keras.datasets.fashion_mnist.load_data() 
    # each image is mapped to one of 10 labels (class names not included)
    classes = ['T-shirt', 'Trouser', 'Pullover', 'Dress', 'Coat', 'Sandal', 'Shirt', 'Sneaker', 'Bag', 'Ankle boot']
    return (x_train, y_train), (x_test, y_test), classes

'''
Vizualize the first 9 images
'''
...
'''
Create CNN model using the sequential API
'''
def create_cnn(num_classes):
    model = tf.keras.Sequential()

    # microarchitecture
    model.add(tf.keras.layers.Conv2D(32, (3, 3), (1, 1), padding='same', activation='relu', input_shape=(28, 28, 1))) 
...
def compile_and_fit(model, x_train, y_train, x_test, y_test, batch_size, epochs, apply_data_augmentation): 
    ...
    if not apply_data_augmentation: 
        ...
        datagen = ImageDataGenerator(rotation_range = 90,           # randomly rotate images in the range (degrees, 0 to 180) 
...
cnn_model, history = compile_and_fit(cnn_model, x_train, y_train, x_test, y_test, batch_size, epochs, apply_data_augmentation)
